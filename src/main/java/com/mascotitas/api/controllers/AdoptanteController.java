...
import com.mascotitas.api.repositories.AdoptanteRepository;
...
public class AdoptanteController {
    ...
    @CrossOrigin
    @PutMapping("/{id}")
    public ResponseEntity<Adoptante> update(@PathVariable Long id,@RequestBody Adoptante update){
        if(!adoptanteRepository.existsById(id)){
            return ResponseEntity.notFound().build();
        }
        update.setId(id);
        Adoptante saved = adoptanteRepository.save(update);
